<?php
session_start();
require_once 'includes/conexion.php';

// Inicializa el mensaje de error
$error = '';

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $input_password = $_POST['password'];

    // Consulta la contraseña desde la base de datos
    $stmt = $pdo->prepare("SELECT contra FROM contraseña_res LIMIT 1");
    $stmt->execute();
    $result = $stmt->fetch();

    // Comparación sin cifrado
    if ($result && $input_password === $result['contra']) {
        header('Location: recepcionista.php');
        exit();
    } else {
        $error = 'Contraseña incorrecta. Intenta de nuevo.';
    }
}
?>

<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <title>Validación de Acceso</title>
    <style>
        body {
            font-family: Arial, sans-serif;
            background-image: url('../img/blur_blue.jpg');
            margin: 0;
            padding: 0;
            display: flex;
            justify-content: center;
            align-items: center;
            height: 100vh;
            flex-direction: column;
        }
        .navbar {
            background-color: #ffa500;
            padding: 15px;
            color: white;
            box-shadow: 0 4px 10px rgba(0, 0, 0, 0.2);
            display: flex;
            justify-content: space-between;
            align-items: center;
            position: fixed;
            width: 100%;
            top: 0;
            left: 0;
            z-index: 1000;
        }
        .navbar .title {
            flex-grow: 1;
            text-align: center;
        }
        .navbar button {
            background-color: #ffa500;
            color: white;
            padding: 10px;
            border: none;
            border-radius: 10px;
            cursor: pointer;
            transition: background-color 0.3s;
        }
        .navbar button:hover {
            background-color: #ffa500;
        }
        .container {
            background: white;
            border-radius: 15px;
            padding: 20px;
            box-shadow: 0 4px 10px rgba(0, 0, 0, 0.2);
            text-align: center;
            max-width: 400px;
            width: 100%;
            margin-top: 80px;
        }
        h2 {
            color: #333;
        }
        input[type="password"] {
            padding: 10px;
            width: 100%;
            border: 1px solid #ddd;
            border-radius: 10px;
            margin: 15px 0;
        }
        button {
            padding: 10px 20px;
            border: none;
            border-radius: 10px;
            background-color: #ffa500;
            color: white;
            font-size: 14px;
            cursor: pointer;
            transition: background-color 0.3s;
        }
        button:hover {
            background-color: #ffa500;
        }
        .error {
            color: red;
            font-size: 14px;
            margin-top: 10px;
        }
    </style>
</head>
<body>
    <div class="navbar">
        <button onclick="location.href='inicio.php'">Volver</button>
        <div class="title">AdminKey Necesaria</div>
    </div>
    <div class="container">
        <h2>Acceso Restringido para Recepción</h2>
        <form method="POST" action="">
            <input type="password" name="password" placeholder="Introduce la contraseña" required>
            <button type="submit">Acceder</button>
            <?php if ($error): ?>
                <p class="error"><?= htmlspecialchars($error); ?></p>
            <?php endif; ?>
        </form>
    </div>
</body>
</html>
